...
openai.api_key = "6c1c61bd992146a1bbcde4a80fef51ba"
openai.api_base = "https://autoagents-global.openai.azure.com/" # your endpoint should look like the following https://YOUR_RESOURCE_NAME.openai.azure.com/
...
def get_llm_response(config, prompt, suffix=None):
    if 'model' in config:
    ...
    if suffix:
        response = openai.Completion.create(**config, prompt=prompt, suffix=suffix)
    else:
...
class GPT_Forward(LLM):
    ...
    def __generate_text(self, prompt, n):
        ...
        while response is None:
            try:
                response = get_llm_response(config, prompt)
    ...
    def __log_probs(self, text, log_prob_range=None):
        ...
        while response is None:
            try:
                response = get_llm_response(config, text)
            except Exception as e:
                print(e)
                print('Retrying...')
                time.sleep(5)
...
class GPT_Insert(LLM):
    ...
    def __generate_text(self, prompt, n):
        ...
        while response is None:
            try:
                response = get_llm_response(config, prompt, suffix)
            except Exception as e:
                print(e)
                print('Retrying...')
                time.sleep(5)
